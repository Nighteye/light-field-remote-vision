...
__global__ void cvl_ip_fista_init_derivative_device( int W, int H,
						     stcflt step,
						     stcflt *uq,
						     ...
						     stcflt *D )
{
  ...
  if ( mask[o] == 0 ) {
    ...
    stcflt uqv = uq[o];
    D[o] = uqv - step * (uqv - f[o]);
  }
}


__global__ void cvl_ip_fista_overrelaxation_device( int W, int H,
						    stcflt alpha,
						    stcflt *u,
						    stcflt *uq )
...
bool coco::tc_inpainting_fista_init( tc_data *data )
{
...
bool coco::tc_inpainting_fista_iteration( tc_data *data )
{
  tc_workspace *w = data->_workspace;
  data->_iteration ++;
  ...
  // constant
  cvl_ip_fista_init_derivative_device<<< w->_dimGrid, w->_dimBlock >>>
    ( data->_W, data->_H,
      1.0 / (w->_lambda * w->_L),
      w->_uq, w->_mask, w->_a,
      w->_f );
  ...
  cvl_ip_fista_overrelaxation_device<<< w->_dimGrid, w->_dimBlock >>>
    ( data->_W, data->_H,
      alpha,
      w->_u_star,
      w->_uq );
