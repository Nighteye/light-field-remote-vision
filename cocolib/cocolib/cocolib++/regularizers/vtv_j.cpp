/* -*-c++-*- */
/** \file regularizer_vtv_j.cu

    Base data structure for the total variation regularizer.
    Reprojection for operator norm (generalized Jacobian).
...
regularizer_vtv_j::regularizer_vtv_j()
{
}
...
bool regularizer_vtv_j::dual_prox( vector_valued_function_2D* P )
{
  assert( P != NULL );
  assert_valid_dims( NULL,P, NULL );
  assert( _N*2 <= P->N() );
  float lambda = get_parameter( "lambda" );
  reprojection_nuclear_norm( P, lambda, 0, _N );
  return true;
}
